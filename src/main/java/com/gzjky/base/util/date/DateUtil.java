package com.gzjky.base.util.date;
...
import com.gzjky.base.util.Validator;
...
public class DateUtil {
	...
	public static Date getDate(String year, String month, String day, String hour, String minute) {
		try {
			DateTime dt = new DateTime(Integer.parseInt(year), Integer.parseInt(month),
					Integer.parseInt(day), Integer.parseInt(hour), Integer.parseInt(minute), 0, 0);
			return dt.toDate();
	...
	public static int validateFromToDate(String fromYear, String fromMonth, String fromDay, String toYear,
		...
		if (!StringUtils.isBlank(fromYear) && !StringUtils.isBlank(toYear)) {
			if (getDate(fromYear, fromMonth, fromDay, "00", "00").after(
				getDate(toYear, toMonth, toDay, "00", "00")))
	...
	public static int validateFromToTime(String fromYear, String fromMonth, String fromDay, String fromHour,
		...
		if (!validateTimeFormat(toYear, toMonth, toDay, toHour, toMinute))
			return RESULT_TO_DATE_ERROR;
		if (!StringUtils.isBlank(fromYear) && !StringUtils.isBlank(toYear)) {
			if (getDate(fromYear, fromMonth, fromDay, fromHour, fromMinute).after(
				getDate(toYear, toMonth, toDay, toHour, toMinute)))
				return RESULT_FROM_DATE_AFTER_TO_DATE;
		}
	...
	public static DateTime parseDate(String str){
		// date parse
		// timestampe long string
		if (Validator.isNumber2(str)) {
			return new DateTime(Long.valueOf(str));
		} else if (Validator.isSlashyyyyMMdd(str)) {
			return DateUtil.parseDate(str, DateFormatter.SDF_YMD9);
		} else if (Validator.isBarsyyyyMMdd(str)) {
			return DateUtil.parseDate(str, DateFormatter.SDF_YMD);
		} else if (Validator.isBarsyyyyMMddHHmmss(str)) {
			return DateUtil.parseDate(str, DateFormatter.SDF_YMDHMS5);
		} else if (Validator.isBarsyyyyMMddHHmmssSSS(str)) {
			return DateUtil.parseDate(str, DateFormatter.SDF_YMDHMS6);
		} else {
